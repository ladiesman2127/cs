...
namespace nonogram_final
{
	public partial class startForm : Form
	{
		...
		{
			List<nonogram_object> objects = new List<nonogram_object>();
			XmlSerializer xmlDeSerializer = new XmlSerializer(typeof(nonogram_object));
			using (FileStream fs = new FileStream("data.xml", FileMode.Open))
			{
				while(fs != null)
				{
					objects.Add(xmlDeSerializer.Deserialize(fs) as nonogram_object);
				}
			}
